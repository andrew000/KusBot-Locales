...
chat_settings--greeting-enabled = ✋ Вітання: { $enabled ->
    [0] ❌
    [1] ✅
    *[2] 🤷‍♂️
}
...
chat_settings--set-greeting-window-text = 💁‍♂️ У цьому вікні ви можете налаштувати вітання нових учасників чату.

    📝 Для встановлення тексту вітання — натисніть кнопку: <blockquote><code>{ chat_settings--set-type-text-button }</code></blockquote>

    🖼 Для встановлення медіа — натисніть кнопку: <blockquote><code>{ chat_settings--set-media-button }</code></blockquote>

    🆔 Для встановлення ID Топіка, куди бот буде надсилати вітання — натисніть кнопку: <blockquote><code>{ chat_settings--set-topic-button }</code></blockquote>
...
chat_settings--greeting-set-type-window-text = 💁‍♂️ У цьому вікні необхідно обрати тип повідомлення, яке бот буде надсилати новим учасникам чату.
...
chat_settings--greeting-set-type-text = ⚙️ Тип вітання: { $greeting_type ->
...
chat_settings--greeting-set-text-window-text = 💁‍♂️ У цьому вікні необхідно відправити текст вітання нових учасників чату.
...
chat_settings--set-text-too-long = ⚠️ Текст занадто довгий.

    💁‍♂️ Максимальна довжина тексту: <code>{ $max_length }</code> символів.

    💡 Введіть інший текст або використайте /cancel для відміни.
chat_settings--greeting-set-media-window-text = 💁‍♂️ У цьому вікні необхідно відправити медіа-файл, який бот буде надсилати новим учасникам чату.
    ...
    — Наліпки не підтримують показ тексту.
chat_settings--set-topic-id-window-text =  💁‍♂️ Тепер відправте будь-яке текстове повідомлення у тому Топіку, куди бот повинен надсилати вітання/прощання учасникам чату.

    💾 Теперішній ID Топіка: <blockquote>{ $stored_topic_id }</blockquote>
chat_settings--reset-topic-id-success = ✅ ID Топіка скинуто.
chat_settings--set-topic-id-success = ✅ ID Топіка збережено:
...
chat_settings--set-topic-id-topic-closed = ⚠️ { $mention }, обраний Топік закритий, а значить бот не зможе надсилати в нього повідомлення.
...
chat_settings--admin-settings-window-text = { chat_settings--admin-settings }

    💁‍♂️ У цьому вікні ви можете налаштувати адміністраторські налаштування чату.
chat_settings--reports-policy-text = <b>{ chat_settings--reports-policy }</b>
...
chat_settings--reports-special-chat-text = 📕 Чат для зберігання скарг: { $is_set ->
        [0] ❌
        [1] ✅
        *[2] 🤷‍♂️
    }
...
chat_settings--no-pending-reports-special-chat-text = ⚠️ Чат для скарг не встановлено.
...
chat_settings--set-reports-special-chat-success-text = ✅ Чат для скарг збережено:

    CHAT_ID: <blockquote><code>{ $chat_id }</code></blockquote>

    💁‍♂️ Тепер ви можете повернутись до вікна налаштувань.
chat_settings--set-reports-special-chat-default-text = 💁‍♂️ Ця команда дозволяє встановити чат для зберігання скарг.
    ...
    — Якщо чат для скарг встановлено — бот буде надсилати повідомлення про скарги у цей чат.
    — Якщо чат для скарг не встановлено — бот буде надсилати повідомлення про скарги у чат, де було надіслано скаргу.
chat_settings--farewell-set-media-window-text = 💁‍♂️ У цьому вікні необхідно відправити медіа-файл, який бот буде надсилати учасникам, які покинули чат.
    ...
    — При скиданні медіа-файлу — скидатиметься той медіа-файл — тип якого обрано в даний момент.
    — Наліпки не підтримують показ тексту.
chat_settings--farewell-set-text-window-text = 💁‍♂️ У цьому вікні необхідно відправити текст прощання з учасниками чату.
    ...
    ♻️ Для скидання тексту до стандартного — натисніть кнопку <blockquote><code>{ chat_settings--reset-button }</code></blockquote>
chat_settings--farewell-set-type-window-text = 💁‍♂️ У цьому вікні необхідно обрати тип повідомлення, яке бот буде надсилати учасникам, які покинули чат.
    ...
    — Повідомлення підтримує форматування. Це означає що роблячи текст курсивом - бот теж надішле його курсивом.
    — Наліпки не підтримують показ тексту.
chat_settings--set-farewell-window-text = 💁‍♂️ У цьому вікні ви можете налаштувати прощання з учасниками чату.

    📝 Для встановлення тексту прощання — натисніть кнопку: <blockquote><code>{ chat_settings--set-type-text-button }</code></blockquote>

    🖼 Для встановлення медіа — натисніть кнопку: <blockquote><code>{ chat_settings--set-media-button }</code></blockquote>

    🆔 Для встановлення ID Топіка, куди бот буде надсилати прощання — натисніть кнопку: <blockquote><code>{ chat_settings--set-topic-button }</code></blockquote>

    ♻️ Для скидання деякого конкретного налаштування — натисніть кнопку: <blockquote><code>{ chat_settings--reset-button }</code></blockquote>

    🗑 Для скидання всіх налаштувань прощання — натисніть кнопку: <blockquote><code>{ chat_settings--reset-all-button }</code></blockquote>
chat_settings--farewell-set-type-text = ⚙️ Тип вітання: { $farewell_type ->
    [text] { chat_settings--set-type-text-button }
    [photo] { chat_settings--set-type-photo-button }
    [video] { chat_settings--set-type-video-button }
    [gif] { chat_settings--set-type-gif-button }
